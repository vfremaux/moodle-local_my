...
function local_my_count_expected_assignments($courseid, $userid) {
    global $DB;
    ...
    $sql = "
        ...
        WHERE
            ...
            a.allowsubmissionfromdata < ? AND
            ass.userid = ?
        GROUP BY
            cm.id
    ";
    // First precheck.
    $params = ['assign', $courseid, time(), $userid];
    if ($rawvisibleassigns = $DB->get_records_sql($sql)) {
        $submitted = 0;
        $total = 0;
