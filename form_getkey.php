...
class GetKey_Form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('html', $OUTPUT->heading(get_string('getlicensekey', $this->_customdata['manager']::$component)));

        $mform->addElement('header', get_string('options', $this->_customdata['manager']::$component));
        ...
        if (!empty($options = $this->_customdata['options'])) {
            ...
            $mform->addHelpButton('activationoptionarr', 'activationoption', $this->_customdata['manager']::$component);

            $this->add_action_buttons(true, get_string('activate', $this->_customdata['manager']::$shortcomponent));

        } else {
            $mess = $OUTPUT->notification(get_string('errornooptions', $this->_customdata['manager']::$component));
            $mform->addElement('static', 'activationoption', get_string('activationoption', $this->_customdata['manager']::$component), $mess);
    ...
    public function validation($data, $files = array()) {
        ...
        if (empty($data['provider'])) {
            $errors['provider'] = get_string('erroremptyprovider', $this->_customdata['manager']::$component);
        }

        if (empty($data['distributorkey'])) {
            $errors['distributorkey'] = get_string('erroremptydistributorkey', $this->_customdata['manager']::$component);
        }
        ...
        if (empty($data['activationoption'])) {
            ...
            if (empty($data['activationoption'])) {
                $errors['activationoption'] = get_string('erroremptyactivationoption', $this->_customdata['manager']::$component);
            }
        }
