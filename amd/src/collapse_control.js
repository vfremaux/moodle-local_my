...
define(['jquery', 'core/config', 'core/log'], function($, config, log) {
    ...
    return {
        ...
        togglecatstate: function(e) {
            ...
            handlesrc = $('#local-my-cathandle-' + catid).attr('src');

            if ($('.local-my-course.cat-' + catid).first().hasClass('collapsed')) {
                $('.local-my-course.cat-' + catid).removeClass('collapsed');
                handlesrc = handlesrc.replace('collapsed', 'expanded');
                $('#local-my-cathandle-' + catid).attr('src', handlesrc);
                hide = 0;
            } else {
                $('.local-my-course.cat-' + catid).addClass('collapsed');
